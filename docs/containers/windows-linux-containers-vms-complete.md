...
> ⚠️ **Windows 10/11 Desktop editions default to Hyper-V Isolation, but Process Isolation is available.** You can opt into Process Isolation with `docker run --isolation=process`, provided the container's base OS build number exactly matches the host's OS build — a mismatch at that level fails immediately with a version error. Docker Desktop uses Hyper-V as the default on desktop editions precisely because that exact build match is rarely guaranteed in practice. Windows Server 2019/2022/2025 also supports Process Isolation and uses it as the default.
